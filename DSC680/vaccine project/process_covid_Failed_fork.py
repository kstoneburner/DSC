...
def build_county_case_death(**kwargs):
    ...
    for FIPS in death_df.sort_values(['FIPS'])['FIPS'].unique():
        ...
        loop_df.loc[loop_df['New_Deaths'] < 0,f'0']=0
        #print(cs)
        #print(ds)
...
def create_weekly_data(df, **kwargs):
    ...
    for date in dates:
        ...
        for FIPS_group in date_df.groupby(FIPS_col):
            ...
            #//*** Rebuild New_Confirm and New Deaths
...
def create_weekly_data_v2(df, **kwargs):
    ...
    print(dates)
    #dates = pd.to_datetime(pd.Series(dates))
    #display(df[df[date_col] == df[date_col].min()])
    #print(len(df))
    ...
    #for date_group in df.groupby('FIPS'):
    for group in df.groupby("FIPS"):
    

        #tdf = group[1].copy()
        #tdf['New_Confirm'] = group[1]['tot_confirm'].diff().fillna(0)
        #tdf['New_Deaths']  = group[1]['tot_deaths'].diff().fillna(0)
            
        #all_df.append(tdf[disp_cols])
