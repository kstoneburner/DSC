...
There are no data with a Cooks Distance greater than one, so none of the data is having an outsized effect on the model.
``` { r cooks_analysis, echo=TRUE}
sum(casewise_df$cooks.distance > 1)
```

Checking leverage indicates a number of outliers. Nearly 5% of the data is attributed to having double the average leverage. Along with 2.5% attributed to triple the average leverage. Taking into account no data has a Cooks Distance greater than one, and the abonormal leverage is limited to 5% the model is still valid.
...
vif() is used to test for no multicollinarity. The vif value and tolerance generate no cause for concern. The mean indicates there may be some bias in the data. The textbook states if the value substantially deviates from 1 there may be bias. There is no reference or scal for substantial.
``` {r vif, echo=TRUE}
...
Plot#1 - Residuals vs fitted. The expected outcome is a random distribution of datapoints. The plotted residuals tend to cluster near the fitted values indicating there is an issue linearaity and randomness in the data.
