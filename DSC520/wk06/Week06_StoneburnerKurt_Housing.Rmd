...
The first selling point of real estate is LOCATION! LOCATION! LOCATION! Let's test that addage with the data.
``` {r its_all_about_location, echo=TRUE }

length(housing_df$zip5)
unique(housing_df$zip5)

```

There are only four zip codes in the data. Clearly, this is not significant.

Let's turn to correlation to find significant values
...
Based on the correlation, variables with a correlation of greater than .20 are good candidates to work with. I was genuinely surprised at the correlation value of bathrooms to sale price. 
